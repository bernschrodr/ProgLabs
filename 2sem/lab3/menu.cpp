...
void Menu::Start()
{
    while (true)
    {
        ...
        while (sscanf(inp.c_str(), "%d", &input) != 1 || input == 0)
        {
            WrongInput(); // выводим сообщение об ошибке
            cin >> inp;   // считываем строку повторно
        }
...
void Menu::MultiplyNum(comp &complexNum)
{
    ...
    while (true)
    {
        ...
        catch (const std::invalid_argument &ia)
        {
            ...
            while (sscanf(inp.c_str(), "%d", &num) != 1 || num == 0)
            {
                WrongInput();      // выводим сообщение об ошибке
                getline(cin, inp); // считываем строку повторно
            }
